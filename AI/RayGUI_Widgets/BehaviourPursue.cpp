...
void Pursue::Update(Agent* agent, float deltaTime) {
	...
	Vector2 originalDistance = Vector2Subtract(GetTargetPos(), agent->GetPos()); // Calculate the distance between the target and the agents position
	Vector2 pursueTarget = Vector2Add(GetTargetVel(), GetTargetPos());			// New target to seek to based on the targets velocity and the prediction amount
